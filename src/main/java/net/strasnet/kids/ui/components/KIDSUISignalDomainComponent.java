 ...
 */
public class KIDSUISignalDomainComponent extends KIDSUIAbstractComponent implements KIDSUIComponent {
	
	public static final org.apache.logging.log4j.Logger logme = LogManager.getLogger(KIDSUISignalDomainComponent.class.getName());
	...
	public KIDSUISignalDomainComponent(IRI myID, KIDSGUIOracle o){
		super(myID, o);
		...
		requiredSubclassOf = IRI.create(TBOXIRI + "#SignalDomain");
		
		logme.debug(String.format("Initialized Signal Domain UI component for %s with: ReqPropChecks: %d, InfPropChecks: %d, ReqDataChecks: %d.", 
				myID,
				reqProps.size(),
				infProps.size(),
				datProps.size()));
