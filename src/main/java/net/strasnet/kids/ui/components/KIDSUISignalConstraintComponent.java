 ...
 */
public class KIDSUISignalConstraintComponent extends KIDSUIAbstractComponent implements KIDSUIComponent {
	
	public static final org.apache.log4j.Logger logme = LogManager.getLogger(KIDSUISignalConstraintComponent.class.getName());
	...
	static {
		infProps.put("#isConstraintOf","#Signal");
//		infProps.put("#isRepresentableBy","#Detector");
	};

	private static final Map<String, KIDSDatatypeClass> datProps = new HashMap<String, KIDSDatatypeClass>();
	static {
		datProps.put("#signalConstraintImplementation", KIDSDatatypeClass.JAVA);
	};
	
	public KIDSUISignalConstraintComponent(IRI myID, KIDSGUIOracle o){
		super(myID, o);
		...
		requiredSubclassOf = IRI.create(TBOXIRI + "#SignalConstraint");
		
		logme.debug(String.format("Initialized Signal Constraint UI component for %s with: ReqPropChecks: %d, InfPropChecks: %d, ReqDataChecks: %d.", 
				myID,
				reqProps.size(),
				infProps.size(),
				datProps.size()));
